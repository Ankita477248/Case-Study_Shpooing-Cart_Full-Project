...
//Automatically connect our custom beans
@Component({
  selector: 'app-card',
...
export class CardComponent implements OnInit, OnDestroy {
  ...
  // Angular call When it initialize the component
  ngOnInit() {
  ...
  // A callback method that perform custom cleanup
  ngOnDestroy(): void {
    ...
    if (this.route.snapshot.url.length == 1) {
      this.productService.getAllInPage(page, size)
        .subscribe(page => {
          ...
          this.page = page;
          this.title = 'Shop with New Year Offer!';
        });
    } else { //  /category/:id
